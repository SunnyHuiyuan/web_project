...
//	}
    ...
    private void edit(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // 4. 响应 updatecustomer_1.jsp 页面: 转发.
    ...
    private void update(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (!oldName.equalsIgnoreCase(name)) {
            ...
            // 2.2 若返回值大于 0, 则响应 updatecustomer_1.jsp 页面: 通过转发的方式来响应 newcustomer_1_1.jsp
            if (count > 0) {
                // 2.2.1 在 updatecustomer_1.jsp 页面显示一个错误消息: 用户名 name 已经被占用, 请重新选择!
                // 在 request 中放入一个属性 message: 用户名 name 已经被占用, 请重新选择!,
                // 在页面上通过 request.getAttribute("message") 的方式来显示
                request.setAttribute("message", "用户名" + name + "已被占用，重新修改");

                // 2.2.2 newcustomer_1_1.jsp 的表单值可以回显.
                // address, phone 显示提交表单的新的值, 而 name 显示 oldName, 而不是新提交的 name

                // 2.2.3 结束方法: return
                request.getRequestDispatcher("/updatecustomer_1.jsp").forward(request, response);
                return;
            }
        }
    ...
    private void add(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // 2.2 若返回值大于 0, 则响应 newcustomer_1.jsp 页面:
        // 通过转发的方式来响应 newcustomer_1.jsp
        if (count > 0) {
            // 2.2.1 要求再 newcustomer_1.jsp 页面显示一个错误消息: 用户名 name 已经被占用, 请重新选择!
            // 在 request 中放入一个属性 message: 用户名 name 已经被占用, 请重新选择!,
            // 在页面上通过 request.getAttribute("message") 的方式来显示
            request.setAttribute("message", "用户名" + name + "已经被占用，请重新填写！");

            // 2.2.2 newcustomer_1.jsp 的表单值可以回显.
            // 通过 value="<%= request.getParameter("name") == null ? "" :
            // request.getParameter("name") %>"
            // 来进行回显
            // 2.2.3 结束方法: return
            request.getRequestDispatcher("/newcustomer_1.jsp").forward(request, response);
            return;
        }
